    ...
    public partial class frmPrincipal : Form
    {
        ...
        private void btnRemover_Click(object sender, EventArgs e)
        {
            if (clbTarefas.Items.Count > 0)
            {
                if (txtTarefas.Text != string.Empty)
                {
                    DialogResult Resp = MessageBox.Show("Remover: " + txtTarefas.Text, "Atenção", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);
                ...
                {
                    DialogResult Resp = MessageBox.Show("Remover: " + clbTarefas.SelectedItem.ToString(), "Atenção", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);
                    if (Resp == DialogResult.Yes)
                    {
